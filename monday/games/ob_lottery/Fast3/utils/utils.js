import {B_S_O_E,SUM, THREE_OF_A_KIND, THREE_NUM, ONE_PAIR,TWO_NUM, GUESS_A_NUM,} from "../constants.js";
 ...
 function getVerticalSlotsActionsButtonsMarkup(index, count,displayText ='All') {
  ...
  function slots_buttons_markup(rowNumber, count,playMethodItemName = '') {

    
    let slotName = ""
    if(playMethodItemName.length !== 0){

      switch (playMethodItemName) {
        case B_S_O_E:
            slotName = slot_buttons_markup_text(count);
          break;
...
// get Lottery List header 
 ...
 function arrayRemove(array, element, sort = false) {
  ...
  export {getRandomNumber,getVerticalSlotsActionsButtonsMarkup,slots_buttons_markup,getSlotsActionsButtonsMarkup,arrayRemove,getDrawLotsMarkup,slot_buttons_markup_text,getListParentTag,getPlayMethodTypesMarkup,playMethodsGroupName,getLotteryListMarkup,getCombinations}